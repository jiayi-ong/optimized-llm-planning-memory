...
| Version | Config key | What it adds over the previous | Why it was added |
|---|---|---|---|
...
- `action` must be a plain string in the format `tool_name({json_args})` — **not** a dict like `{"tool_name": ..., "arguments": ...}`. The parser regex in `_parse_response()` matches `(\w+)\((\{.*?\})\)` — only the `tool_name({...})` surface form triggers a successful parse. A dict format prints as Python repr and is never matched, causing a parse failure.
- `observation` must be a Python repr string matching `TrajectoryModel.to_text()` output (single-quoted keys, not JSON). The observation is not re-parsed — it is injected verbatim. If the few-shot examples use JSON-style double-quoted keys, the agent may produce misformatted observations in its own reasoning.
...
| `MCTS_COMPRESSOR` | The last `CompressedState` (6 sections) **plus** an additional `[PLANNING LOOKAHEAD]` section injected by `ContextBuilder._build_memory_section_mcts_compressor()`. The lookahead section contains `MCTSTreeRepresentation.top_candidates` (top-K candidate action sequences with simulated Q-values) and `tradeoffs` (per-candidate constraint coverage summary) derived from the MCTS search that ran at the last compression event. |

**How MCTS_COMPRESSOR works end-to-end:**
1. At each compression event, `ReActAgent` calls `MCTSController.search(trajectory)` → `MCTSTreeRepresentation`.
2. The tree rep is forwarded to `compressor.compress_with_tree(trajectory, mcts_tree)` (requires an `MCTSAwareCompressor` subclass).
3. The resulting `CompressedState` has `top_candidates` and `tradeoffs` fields populated from the tree.
4. `ContextBuilder._build_memory_section_mcts_compressor()` renders both the standard 6-section state and the `[PLANNING LOOKAHEAD]` appendix.

Config pairing: always use `agent=react_mcts` with `compressor=llm_mcts` or `compressor=mcts_gat`. Using these configs independently logs a startup warning and silently falls back to standard compression.
...
Compression triggers on whichever condition is satisfied first:
...
**Why a two-condition OR?** The step-count trigger ensures regular compression cadence regardless of trajectory verbosity (useful during training where episode length varies widely). The token-count trigger guards against unusually long observations (e.g., a single tool call returning a large hotel list) that could blow the context budget before the step threshold is reached. Both can fire in the same step — the compressed state is produced once and injected for the remainder of the episode window.

**Training vs. inference frequency:** During PPO training with `compress_every_n_steps=5` and a 4-env vectorized setup, a compression event occurs every 5 agent steps. Each compression event corresponds to one gymnasium `step()` — meaning the PPO policy gets one update signal per 5 ReAct steps. Increasing `compress_every_n_steps` means larger observation windows (the compressor sees more context before acting) but fewer PPO updates per episode. See [docs/TRAINING.md — How the RL Loop Works](TRAINING.md#how-the-rl-loop-works) for the full episode framing.
